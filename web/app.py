...
def hello_world(path=None):
    ...
    if path == 'recipe-tag-index.html':
        ...
        return render_template('{}'.format(path), disease=disease, nameList=nameList)
...
def query():
    from query import query, temp, getRecommend, selectFood, selectIngre
    ...
    if n == "1" : # 먹고 싶은 음식 있을 때
        if len(data) == 1 or len(data) == 2:
            ...
            resp = make_response(render_template('index.html', dname1=data[0], dname2=data[1], recommend1=recommend1,
                                                 ...
                                                 rate=recommend_rate, data=q, foodName = foodName, pageNum = pageNum))
            resp.set_cookie('a', a)
            resp.set_cookie('choice', n)
            if foodName:
                resp.set_cookie('foodName', foodName)
        ...
        if len(data) == 1 or len(data) == 2:
            ...
            resp = make_response(render_template('index2.html', dname1=data[0], dname2=data[1], recommend1=recommend1,
                                                 recommend2=recommend2, caution1=caution1, caution2=caution2,
                                                 rate=recommend_rate, data=q, pageNum = pageNum))
            resp.set_cookie('a', a)
            resp.set_cookie('choice', n)
            return resp
